...
def build_keypoints(count=16, width=0.25, x_offset=0.3, z_offset=0.25):
    ...
    """
    t = np.linspace(0, 2 * np.pi, num=count, endpoint=False)
    y, z = get_lemniscate_keypoint(t, a=width)
    cords = np.column_stack([np.full(count, fill_value=x_offset), y, z + z_offset])
    return cords
...
def ik_track(model, data, site_name, target_pos,
    ...
    for i in range(max_iters):
        ...
        qdot = J.T @ np.linalg.solve(J @ J.T + damping * np.eye(6), weighted_err)
