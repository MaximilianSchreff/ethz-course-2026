...
def process_action(action: np.ndarray, jnt_range: np.ndarray) -> np.ndarray:
    ...
    """
    mids = (jnt_range[:, 0] + jnt_range[:, 1]) / 2
    halfs = jnt_range[:, 1] - mids
    return mids + action * halfs
...
def get_obs(
    ...
    """
    ee_pos_relative = ee_pos_w - base_pos_w
    ee_quat_relative = quat_normalize(rot_mat_to_quat(base_rot_w.T @ ee_rot_w))
    target_pos_relative = target_pos_w - base_pos_w

    return np.concatenate([qpos, ee_pos_relative, ee_quat_relative, target_pos_relative])
